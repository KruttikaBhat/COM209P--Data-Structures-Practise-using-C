...
void sort(struct node *ptr)     //to sort the node when it is not yet full
{
    int temp;
...
void arrsort(int a[],int n)        //to sort the 4 keys when a bucket overflows
{
    int i,temp,j;
    for(i=1;i<n;i++)
    {
...
struct node * splitparent(struct node *pptr,struct node *ptr,struct node *newpptr,struct node *new)
{
    ...
    if(ptr==pptr->child1)                                 //when the ptr is in the first position, new will be inserted in 2nd position
    {
    ...
    else if(ptr==pptr->child2)                       //when the ptr is in the 2nd position, new will be inserted in 3nd position                      
    {
        newpptr->child1=new;
        new->parent=newpptr;
    ...
    else if(ptr==pptr->child3)                     //when the ptr is in the 3rd position, both will become the first 2 children of newpptr
    {
        newpptr->child1=ptr;
        ptr->parent=newpptr;
...
void insert(struct node *ptr,int x)
{
    ...
    if(ptr->count==2)                  //if the bucket has only 2 children then just insert and sort
    {
        ptr->key3=x;
    ...
    if(ptr->count==3)    //if the bucket is already full
    {
        ...
        while(pptr)
        {
            ...
            if(pptr->count==2)    //if the parent only has 2 children then we can add the new as it's child
            {
                ...
                else if(ptr==pptr->child2)    //when new bucket becomes the third child of parent
                {
                    pptr->child3=new;
                }

                struct node *temp=pptr->child2;
                while(temp->type!=2)         //get key1 value for pptr
                    temp=temp->child1;
                pptr->key1=temp->key1;     //bring in concept of type to traverse to get the key value
                temp=pptr->child3;
                while(temp->type!=2)         //get key2 value for pptr
                    temp=temp->child1;
                pptr->key2=temp->key1;
                new->parent=pptr;
                ptr->parent=pptr;
            ...
            else if(pptr->count==3)      //if the parent already has 3 children
            {
                ...
                struct node *newpptr;                 //split parent and associate respective children with pptr and newpptr
                newpptr=splitparent(pptr,ptr,newpptr,new);
                ptr=pptr;                            //now update the pointers ptr, new and pptr
                new=newpptr;
                pptr=ptr->parent;                            
                if(pptr==NULL)                   //check whether there is already some parent existing to attach the new parent, pptr is NULL implies that there is no parent so a new head has to be created
                {
                    struct node *newhead=(struct node *)malloc(sizeof(struct node));
                
                    newhead->child1=ptr;           //assign values to newhead
                    newhead->child2=new;
                    newhead->child3=NULL;
                    ptr->parent=newhead;
                    new->parent=newhead;
                    ...
                    struct node *temp=newhead->child2;     //get the key values
                    while(temp->type!=2)
                    {
                        temp=temp->child1;
                    }
            ...
            }                //if pptr is not NULL then we go back to the beginning of the while loop and check whether pptr has 2 children or 3, if 3 we have to split again, if 2 we just associate new to pptr and return 
...
void display(struct node *ptr)             //recursive diplay function, will first display children keys then parent keys
{
    if(ptr)
    {
        display(ptr->child1);
